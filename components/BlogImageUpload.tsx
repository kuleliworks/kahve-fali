...
const final: string =
  json.publicUrl || json.url || (json.key ? `/media/${json.key}` : "");
      ...
      setBusy(false);
      e.currentTarget.value = ""; // aynı dosyayı tekrar seçebilsin
    }
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        {preview && (
          <a href={preview} target="_blank" rel="noopener" className="text-sm underline underline-offset-4">
            Görseli aç
          </a>
        )}
      </div>
      ...
      <p className="text-xs text-stone-500">
        Desteklenen: JPG/PNG/WebP (≤ 12MB). Yükleme sonrası URL otomatik eklenir.
      </p>
    </div>
  );
}
